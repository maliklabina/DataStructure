...
void sort_SGPA(int l, int k)  // sorting SGPA using quick sort
{
    int r = k - 1;
    if (l >= r) return; // Base case: if the partition size is 0 or 1, return

    int i = l;
    int j = r;
    struct student prec = s1[l]; // Store the pivot record
    int p = s1[l].SGPA; // Select pivot element (SGPA)

    while (i < j)
    {
        // Move i to the right until an element larger than pivot is found
        while (s1[i].SGPA <= p && i < r) i++;

        // Move j to the left until an element smaller than pivot is found
        while (s1[j].SGPA > p && j > l) j--;

        if (i < j) // Swap the elements if i < j
        {
    ...
    s1[l] = s1[j]; // Place the pivot in its correct position
    s1[j] = prec;

    // Recursive calls to sort the partitions
    sort_SGPA(l, j - 1); // Sort left partition
    sort_SGPA(j + 1, r); // Sort right partition
}
